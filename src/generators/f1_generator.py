"""Generate F1 results and JSON summaries"""
...
class F1JSONGenerator:
    """Generate F1 results in JSON format"""

    def __init__(self):
        self.results_dir = Path("results/f1")
        self.results_dir.mkdir(parents=True, exist_ok=True)

    def generate_summary(self, rare_performances, archive_summary):
        """Generate overall F1 summary"""
        ...
        if total_rare == 0:
            ...
            summary = {
                ...
                "rarest_performance": {
                    "driver_name": rarest['driver_name'],
        ...
        # Save summary
        summary_path = self.results_dir / "f1_summary.json"
        with open(summary_path, 'w') as f:
            json.dump(summary, f, indent=2)

        logger.success(f"F1 summary saved to {summary_path}")
        return summary

    def generate_detailed_results(self, rare_performances):
        """Generate detailed F1 results JSON"""
        detailed = {
        ...
        detailed_path = self.results_dir / "f1_detailed.json"
        with open(detailed_path, 'w') as f:
            json.dump(detailed, f, indent=2)

        logger.success(f"Detailed F1 results saved to {detailed_path}")
        return detailed

    def generate_all(self, rare_performances, archive_summary):
        """Generate all F1 result files"""
        logger.info("Generating F1 result files...")
