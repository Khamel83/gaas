"""Generate MLB results and JSON summaries"""
...
class MLBJSONGenerator:
    """Generate MLB results in JSON format"""
    ...
    def generate_summary(self, rare_performances, archive_summary):
        ...
        # Save summary
        summary_path = self.results_dir / "mlb_summary.json"
        with open(summary_path, 'w') as f:
            json.dump(summary, f, indent=2)

        logger.success(f"MLB summary saved to {summary_path}")
        return summary

    def generate_detailed_results(self, rare_performances):
        """Generate detailed MLB results JSON"""
        detailed = {
        ...
        detailed_path = self.results_dir / "mlb_detailed.json"
        with open(detailed_path, 'w') as f:
            json.dump(detailed, f, indent=2)

        logger.success(f"Detailed MLB results saved to {detailed_path}")
        return detailed

    def generate_all(self, rare_performances, archive_summary):
        """Generate all MLB result files"""
        logger.info("Generating MLB result files...")
